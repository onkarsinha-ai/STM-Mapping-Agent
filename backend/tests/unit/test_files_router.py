...
@pytest.mark.asyncio
async def test_extract_schema_csv_success():
...
@pytest.mark.asyncio
async def test_extract_schema_unsupported_format():
...
@pytest.mark.asyncio
async def test_extract_schema_empty_file():
...
@pytest.mark.asyncio
async def test_extract_schema_file_too_large():
...
@pytest.mark.asyncio
async def test_extract_schema_no_filename():
...
@pytest.mark.asyncio
async def test_extract_schema_parse_failure():
    """Test generic parse failure returns 422."""
    with patch("app.routers.files.SchemaExtractor") as mock_extractor_cls:
        mock_extractor = MagicMock()
        mock_extractor.extract.side_effect = Exception("corrupt data")
        mock_extractor_cls.return_value = mock_extractor
